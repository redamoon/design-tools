...
export async function loadTokensFromJson(path: string): Promise<Token[]> {
    ...
    // Parses format compatible with Figma Tokens / Token Studio
    ...
    function walk(obj: any, prefix = '') {
        for (const k of Object.keys(obj)) {
            ...
            // Check if this node is a token (has a 'value' property)
            if (val && typeof val === 'object' && 'value' in val) {
                const rawValue = val.value;
                const type = val.type || (typeof rawValue === 'string' && rawValue.startsWith('#') ? 'color' : 'other');
                tokens.push({
                    type,
                    name,
                    rawValue,
